...
SELECT 'HELLO', 'WORLD'
FROM DUAL;
-- 원래 1개 rOW 1개 col인데 내가 셀렉트한 게 그 이상이면 마음대로 늘어남?
...
-- EMPLOYEE 테이블에서 모든 사원의 이메일과 이메일 글자 수(별칭 글자 수) 조회
SELECT EMAIL, LENGTH(EMAIL) "글자 수"
FROM EMPLOYEE;
...
SELECT INSTR('AABAACAABBAA', 'B', 1, 2) FROM DUAL; -- 2번째 인자값이 (4번째 인자값) 번째로 등장한 값의 위치
SELECT INSTR('AABAACAABBAA', 'B', -1) FROM DUAL; -- -1은 뒤에서 첫번째 부터 탐색을 의미
SELECT INSTR('AABAACAABBAA', 'B', -1, 3) FROM DUAL; -- 뒤에서부터 (4번째 인자값) 번째로 등장한 2번째 인자값의 위치

/*
    3) LPAD / RPAD
        - LPAD/RPAD(컬럼|'문자값', 길이(글자 개수)[, '덧붙이려고 하는 문자'])
...
SELECT TRIM(LEADING 'Z' FROM 'ZZZKHZZZ') FROM DUAL; -- 앞쪽
SELECT TRIM(TRAILING 'Z' FROM 'ZZZKHZZZ') FROM DUAL; -- 뒤쪽
...
-- EMPLOYEE 테이블에서 모든 사원의 이메일과 '@' 전까지 이메일 글자 수 조회
SELECT EMAIL, LENGTH(SUBSTR(EMAIL, 1, INSTR(EMAIL, '@')-1))
FROM EMPLOYEE;
...
SELECT MOD(-10, 3) FROM DUAL;
SELECT MOD(10, -3) FROM DUAL; -- 두번째 인자값의 음수는 무관하다...?
...
-- 연도 형식이 YY가 아니라 RR인 점: RR은 1950년부터 100년을 기준으로 범위가 지정되어있음
...
SELECT '1234' ZZZZZZZZZZZZZZZZ FROM DUAL; -- 문자열: 왼쪽에 붙음
SELECT 1234 ZZZZZZZZZZZZZZZZZ FROM DUAL; -- 넘버: 오른쪽에 붙음
...
SELECT TO_CHAR(1234, 'L999') FROM DUAL; -- 깨짐
...
-- 날짜 --> 문자
SELECT SYSDATE FROM DUAL; -- 차이: 
SELECT TO_CHAR(SYSDATE) FROM DUAL; -- 차이 확인: 값을 더블클릭하고 연필모양 확인
...
-- 문자 --> 날짜
SELECT TO_DATE('20211014') FROM DUAL;
SELECT TO_DATE('20211014 19 08 30') FROM DUAL;
SELECT TO_DATE('2021/10/14', 'YYYY/MM/DD') FROM DUAL; -- 형식 지정 가능
...
SELECT EMP_NAME 사원명
    , BONUS 보너스
    , SALARY*12 + SALARY*NVL(BONUS, 0) "보너스가 포함된 연봉"
FROM EMPLOYEE;
-- EMPLOYEE 테이블에서 사원명, 부서 코드 조회
SELECT EMP_NAME 사원명
    , NVL(DEPT_CODE, '부서없음') "부서 코드"
FROM EMPLOYEE;
-- 보너스 일괄적용 하여 사원이름, 보너스, 연봉 조회(보너스 있는 사람들은 무조건 10%로 적용, 없으면 안줌)
SELECT EMP_NAME 사원명
    , BONUS 보너스
    , SALARY*12 + SALARY*NVL2(BONUS, 0.1, 0) 연봉
FROM EMPLOYEE;
...
SELECT EMP_NAME 사원명
    , EMP_NO 주민번호
    , DECODE(SUBSTR(EMP_NO, 8, 1), 2, '여자', 1, '남자') 성별
FROM EMPLOYEE;
...
SELECT SUM(SALARY)
FROM EMPLOYEE;
-- SELECT EMP_NAME, SUM(SALARY) : 오류 (이유: 사원명 컬럼과 합계 컬럼의 행 개수가 다름)
-- 결과집합(RESULT SET)은 무조건 사각형 형태여야 함
...
SELECT AVG(BONUS)
FROM EMPLOYEE; -- NULL 값: 아예 취급하지 않고 처리함